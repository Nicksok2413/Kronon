...
class TestClientHistory(BaseAPITest):
    ...
    async def test_history_initiator_sync(self, admin_client: AsyncClient, admin_user: User):
        ...
        Args:
            admin_client (AsyncClient): Авторизованный асинхронный клиент (с правами админа).
            admin_user (User): Административный пользователь.
        """
    ...
    async def test_initiator_logging_with_details(self, system_client: AsyncClient, settings: Any):
        """
        Проверка: лог инициатора запроса к API содержит User-Agent, если флаг включен.

        Args:
            system_client (AsyncClient): Авторизованный асинхронный клиент (с системным API-ключом).
            settings (Any): Настройки (для включения флага детального логирования).
        """

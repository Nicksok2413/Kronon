...
async def get_client_queryset(user_id: UUID, is_admin: bool) -> SoftDeleteQuerySet[Client]:
    """
    Возвращает базовый QuerySet для списка клиентов с оптимизацией и с учетом прав доступа (OLP).
    ...
    return (
        ...
        .for_user(user_id, is_admin)  # OLP-фильтрация в БД
        .select_related(
            "department",
            "accountant",
            "primary_accountant",
...
async def get_client_by_id(client_id: UUID, user_id: UUID, is_admin: bool) -> Client | None:
    """
    ...
    try:
        # Базовый QuerySet с OLP-фильтрацией
        queryset = await get_client_queryset(user_id=user_id, is_admin=is_admin)

        # .afirst() вместо .aget(), чтобы избежать исключения DoesNotExist и вернуть None
        client = await queryset.filter(id=client_id).afirst()
